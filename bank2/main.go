...
var parallelStmts = flag.Bool("parallel-stmts", false, "Should independent statements be run in parallel?")
...
func moveMoney(db *sql.DB, aggr *measurement) {
	...
	for !transfersComplete() {
		...
		if err := crdb.ExecuteTx(db, func(tx *sql.Tx) error {
			...
			suffix := ""
			if *parallelStmts {
				suffix = ` RETURNING NOTHING`
			}
			insert := fmt.Sprintf(`INSERT INTO transaction (id, txn_ref) VALUES ($1, $2)%s;`, suffix)
			txnID := atomic.AddInt32(&txnCount, 1)
			_, err = tx.Exec(insert, txnID, fmt.Sprintf("txn %d", txnID))
			if err != nil {
				return err
			}
			insert = fmt.Sprintf(`INSERT INTO transaction_leg (account_id, amount, running_balance, txn_id) VALUES ($1, $2, $3, $4)%s;`, suffix)
			...
			update := fmt.Sprintf(`UPDATE account SET balance = $1 WHERE id = $2%s;`, suffix)
