...
func shrink(e sqlExecutor, inodeID, from, to uint64) error {
	...
	delStmt := `DELETE FROM fs.BLOCK WHERE id = $1 AND block >= $2`
...
func grow(e sqlExecutor, inodeID, from, to uint64) error {
	...
	for i := insertFrom; i < insertTo; i++ {
		paramStrings = append(paramStrings, fmt.Sprintf("(%d, %d, $1)", inodeID, i))
	}
	...
	if addRange.lastLength > 0 {
		...
		paramStrings = append(paramStrings, fmt.Sprintf("(%d, %d, $%d)",
			inodeID, addRange.last, count))
		count++
	}
	...
	insStmt := fmt.Sprintf(`INSERT INTO fs.block VALUES %s`, strings.Join(paramStrings, ","))
